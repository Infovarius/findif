...
int read_snap(char *snapname)                 //returns code of error
{
    int error = 0;
    int i, k, l, tmpr;
    int rank1 = rank;
    ...
    fileclose(inp);
    if (error) nrerror("Main flow couldn't have been read from file!!!", -1, error);
