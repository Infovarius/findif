   ...
   for(k=ghost;k<mm3;k++)
     {
      ...
      df[1][i][j][k]=nut[i][j][k]*(dv2[1][0]+dv2[1][1]+dv2[1][2])
		     -dp1[0]    +2*W*f[2][i][j][k]  //+w*w/r_1[i]                 //forces of inertion
//                     +(j+n[2]<=ghost+2 ? coordin(k,2)*f[2][i][j][k] : 0)                //helical force
		     -f[1][i][j][k]*dv1[1][0]
		     -f[2][i][j][k]*dv1[1][1]
		     -f[3][i][j][k]*dv1[1][2]
		     ;
      df[2][i][j][k]=nut[i][j][k]*(dv2[2][0]+dv2[2][1]+dv2[2][2])
		     -dp1[1]    -2*W*f[1][i][j][k]  //-dw/r_1[i]                 //forces of inertion
//                     -(j+n[2]<=ghost+2 ? (coordin(i,0)-rc)*f[2][i][j][k] :0)            //helical force
		     -f[1][i][j][k]*dv1[2][0]
		     -f[2][i][j][k]*dv1[2][1]
		     -f[3][i][j][k]*dv1[2][2]
		     ;
      df[3][i][j][k]=nut[i][j][k]*(dv2[3][0]+dv2[3][1]+dv2[3][2])
		     -dp1[2]
		     -f[1][i][j][k]*dv1[3][0]
		     -f[2][i][j][k]*dv1[3][1]
		     -f[3][i][j][k]*dv1[3][2];
      df[0][i][j][k]= -(dv1[1][0]+dv1[2][1]+dv1[3][2])/Gamma;
   if(fabs(coordin(i,0))<=rF && fabs(coordin(j,1))<=rF)
	df[3][i][j][k] += aF;
...
/*----------------------- filling of ghost nodes ------------------------------*/
  ...
  if(pr_neighbour[0]==-1)
	...
	for(k=ghost;k<mm3;k++)
		{
		f[0][i][j][k] = + f[0][2*ghost-i-1][j][k];
		f[1][i][j][k] = - f[1][2*ghost-i-1][j][k];
		f[2][i][j][k] = - f[2][2*ghost-i-1][j][k];
		f[3][i][j][k] = - f[3][2*ghost-i-1][j][k];
		}
  ...
  if(pr_neighbour[1]==-1)
	...
	for(k=ghost;k<mm3;k++)
		{
		f[0][i][j][k] = + f[0][2*mm1-i-1][j][k];
		f[1][i][j][k] = - f[1][2*mm1-i-1][j][k];
		f[2][i][j][k] = - f[2][2*mm1-i-1][j][k];
		f[3][i][j][k] = - f[3][2*mm1-i-1][j][k];
		}
  ...
  if(pr_neighbour[2]==-1)
	...
	for(k=ghost;k<mm3;k++)
		{
		f[0][i][j][k] = + f[0][i][2*ghost-j-1][k];
		f[1][i][j][k] = - f[1][i][2*ghost-j-1][k];
		f[2][i][j][k] = - f[2][i][2*ghost-j-1][k];
		f[3][i][j][k] = - f[3][i][2*ghost-j-1][k];
		}
  ...
  if(pr_neighbour[3]==-1)
	...
	for(k=ghost;k<mm3;k++)
		{
		f[0][i][j][k] = + f[0][i][2*mm2-j-1][k];
		f[1][i][j][k] = - f[1][i][2*mm2-j-1][k];
		f[2][i][j][k] = - f[2][i][2*mm2-j-1][k];
		f[3][i][j][k] = - f[3][i][2*mm2-j-1][k];
		}
  ...
  if(pr_neighbour[4]==-1)
	...
	for(k=0;k<ghost;k++)
		{
		f[0][i][j][k] = + f[0][i][j][2*ghost-k-1];
		f[1][i][j][k] = - f[1][i][j][2*ghost-k-1];
		f[2][i][j][k] = - f[2][i][j][2*ghost-k-1];
		f[3][i][j][k] = - f[3][i][j][2*ghost-k-1];
		}
  ...
  if(pr_neighbour[5]==-1)
	...
	for(k=mm3;k<m3;k++)
		{
		f[0][i][j][k] = + f[0][i][j][2*mm3-k-1];
		f[1][i][j][k] = - f[1][i][j][2*mm3-k-1];
		f[2][i][j][k] = - f[2][i][j][2*mm3-k-1];
		f[3][i][j][k] = - f[3][i][j][2*mm3-k-1];
		}
...
/*   for(i=0;i<m1;i++)
   ...
   for(k=0;k<m3;k++)
       {
     //regions
        if(i>=ghost && i<mm1 && k>=ghost && k<mm3) setType(&node[i][k],NodeFluid);
     //for hydrodynamics
...
//      if(isType(node[i][k],NodeFluid))
        {
	f[0][i][j][k]=0;
	f[1][i][j][k]=0/*NoiseNorm*cos(2*M_PI*coordin(j,1)/R)*sin(2*M_PI*coordin(k,2)/R)
		      + Noise*((double)rand()-RAND_MAX/2)/RAND_MAX*
		       (R*R-pow(coordin(i,0),2) - pow(coordin(k,2),2))/R/R*/;
	f[2][i][j][k]=0/*(parabole+Noise*((double)rand()-RAND_MAX/2)/RAND_MAX)*
		       (R*R-pow(coordin(i,0),2) - pow(coordin(k,2),2))/R/R*/;
	f[3][i][j][k]=0/*NoiseNorm*sin(2*M_PI*coordin(j,1)/R)*sin(2*M_PI*coordin(k,2)/R)
		      + Noise*((double)rand()-RAND_MAX/2)/RAND_MAX*
		       (R*R-pow(coordin(i,0),2) - pow(coordin(k,2),2))/R/R*/;
	}
   for(i=0;i<m1;i++)
                      ...
                      //0-r,1-phi,2-z
{
 switch (dir)
 {
   ...
   case 1:  return dx[dir]*(i-ghost+0.5+n[dir])-R;
